...
#define ALLOC_EXPR_TYPE_ZERO(member) \
  wabt_alloc_zero(offsetof(WabtExpr, member) + sizeof(((WabtExpr*)0)->member))
...
#define DEFINE_NEW_EXPR(type_, name, member)         \
  WabtExpr* wabt_new_##name##_expr(void) {           \
    WabtExpr* result = ALLOC_EXPR_TYPE_ZERO(member); \
    result->type = type_;                            \
    return result;                                   \
  }
...
#define DEFINE_NEW_EMPTY_EXPR(type_, name)         \
  WabtExpr* wabt_new_##name##_expr(void) {         \
    WabtExpr* result = ALLOC_EXPR_TYPE_ZERO(next); \
    result->type = type_;                          \
    return result;                                 \
  }
...
WabtExpr* wabt_new_empty_expr(WabtExprType type) {
  WabtExpr* result = ALLOC_EXPR_TYPE_ZERO(next);
  result->type = type;
  return result;
}
