...
static WabtResult on_function_signatures_count(uint32_t count,
                                               void* user_data) {
  Context* ctx = user_data;
  wabt_reserve_func_ptrs(&ctx->module->funcs, count);
  return WABT_OK;
}
...
static WabtResult on_table_count(uint32_t count, void* user_data) {
  Context* ctx = user_data;
  wabt_reserve_table_ptrs(&ctx->module->tables, count);
  return WABT_OK;
}
...
static WabtResult on_memory_count(uint32_t count, void* user_data) {
  Context* ctx = user_data;
  wabt_reserve_memory_ptrs(&ctx->module->memories, count);
  return WABT_OK;
}
...
static WabtResult on_global_count(uint32_t count, void* user_data) {
  Context* ctx = user_data;
  wabt_reserve_global_ptrs(&ctx->module->globals, count);
  return WABT_OK;
}
